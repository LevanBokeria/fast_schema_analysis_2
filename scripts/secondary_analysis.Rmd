...
# Learning parameter estimate BFs
...
# Comparing all rough performance variables


## Plots: 

```{r compare-all-depvar-plots, fig.width=15, fig.height=10}
data_summary_long_performance_types <- data_summary %>%
        pivot_longer(cols = c(rep_1_to_2_diff,
                              rep_2_mouse_error_mean,
                              rep_2_3_4_mouse_error_mean,
                              block_2_mouse_error_mean),
                     names_to = 'performance_type',
                     values_to = 'mean_error') %>% 
        mutate(performance_type = as.factor(performance_type)) %>%
        reorder_levels(performance_type, order = c('rep_1_to_2_diff',
                                                   'rep_2_mouse_error_mean',
                                                   'rep_2_3_4_mouse_error_mean',
                                                   'block_2_mouse_error_mean'))


data_summary_long_performance_types %>%
        filter(hidden_pa_img_type == 'all_pa') %>%
        droplevels() %>%
        ggplot(aes(x=condition,
                   y=mean_error,
...
```{r compare-all-dep-var-bf}

# CODE BELOW IS MESSY AND INCOMPLETE
...
data_summary_long_performance_types %>%
        ...
        pivot_wider(id_cols = c(ptp,
                    ...
                    values_from = mean_error) %>% 
        mutate(schema_l_minus_schema_c = schema_ic - schema_c) %>% 
        group_by(performance_type) %>%
        
        summarise(bf = reportBF(ttestBF(cur_data()$schema_l_minus_schema_c)[1],4)
        ) %>%
        ungroup() %>% View()        
...
data_for_bf_near_l_vs_c <- data_summary %>%
        ...
        pivot_wider(id_cols = c(ptp,
                    ...
                    names_from = condition,
                    values_from = learning_rate_two_param) %>% 
        mutate(schema_l_minus_schema_c = schema_c - schema_l) %>% 
